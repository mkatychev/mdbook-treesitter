//! This module handles fetching the [`tree_sitter::Language`] struct, which represents a language grammar configuration

const LANGUAGE_DIR: &str = "TREESITTER_LANGUAGE_DIR";
...
impl Language {
    pub fn new(name: String, grammar: Grammar) -> Self {
        Self { name, grammar }
    }
    ...
    pub fn find_query_file(&self) -> TopiaryConfigResult<PathBuf> {
        ...
        let potentials: [Option<PathBuf>; 4] = [
            std::env::var(LANGUAGE_DIR).map(PathBuf::from).ok(),
            option_env!(LANGUAGE_DIR).map(PathBuf::from),
...
impl GitSource {
    ...
    pub fn fetch_and_compile_with_dir(
        ...
        log::info!("{name}: Checking out {} {}", name, self.git, self.rev);
        checkout(
            &mut index,
            &tmp_dir,
            repo.objects.clone(),
